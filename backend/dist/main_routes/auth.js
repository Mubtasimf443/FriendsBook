...
router.post("/create-registration-session", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const validationResult = yield auth_schema_1.registrationUserSchema.safeParseAsync(req.body);
            ...
            let existingUser = yield user_1.User.findOne({ email: userData.email });
            if (existingUser) {
                return res.status(400).json({
                    success: false,
            ...
            const session = yield temporarySession_1.default.create({
                name: temporarySession_1.TemporarySessionNames.REGISTRATION_SESSION,
                key: sessionKey,
                value: JSON.stringify(Object.assign({ hasOtpRequest: 10 }, userData))
            });
...
router.post("/request-registration-otp", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let validationResult = yield auth_schema_1.tempSessionValidation.safeParseAsync(req.body.sessionKey);
            if (!validationResult.success) {
                return res.status(400).json({
                    success: false,
                    message: validationResult.error.errors[0].message,
            ...
            // Find the session
            const session = yield temporarySession_1.default.findOne()
            ...
            session.value = JSON.stringify(Object.assign(Object.assign({}, userData), { otp, otpExpiry: Date.now() + 70 * 1000 // OTP valid for 1 minutes 25 seconds
             }));
            ...
            // Send OTP via email
            const emailSent = yield auth_emails_1.authEmails.registrationOtpEmail(otp, userData.email);
            if (!emailSent) {
                return res.status(500).json({
                    success: false,
...
router.post("/verify-registration-otp", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let validationResult = yield auth_schema_1.VerifyOtpSchema.safeParseAsync(req.body);
            if (!validationResult.success) {
                return res.status(400).json({
                    success: false,
                    message: validationResult.error.errors[0].message,
            ...
            let existingUser = yield user_1.User.findOne({ email: sessionData.email });
            if (existingUser) {
                return res.status(400).json({
                    success: false,
                    message: "Email is already registered",
            ...
            existingUser = yield user_1.User.findOne({ "phoneInfo.number": sessionData.phoneInfo.number, "phoneInfo.country.phone_code": sessionData.phoneInfo.country.phone_code });
            if (existingUser) {
                return res.status(400).json({
                    success: false,
            ...
            let passwordHash = yield (0, auth_controller_1.hashPassword)(sessionData.password, passwordSalt);
            ...
            // Create auth session
            yield AuthSession_1.default.create({
                key: authToken,
...
router.post('/is-registration-successfull', function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (authToken) {
                let session = yield AuthSession_1.default.findOne({ key: authToken });
                if (session) {
                    let userId = session === null || session === void 0 ? void 0 : session.value.userId;
                    user = yield user_1.User.findById(userId);
                }
                if (!user) {
                    res.status(400).json({
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (emailRegex.test(credential)) {
                ...
                const validateEmail = yield schemaComponents_1.emailValidatior.safeParseAsync(credential);
                ...
                existingUser = yield user_1.User.findOne({ email: credential });
            }
            else {
            ...
            const isPasswordValid = yield (0, auth_controller_1.comparePasswords)({
                password: password,
            ...
            yield AuthSession_1.default.deleteOne({ 'value.email': existingUser.email });
...
router.post('/reset-password', function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let validationResult = yield auth_schema_1.ResetPasswordSchema.safeParseAsync(req.body);
            if (!validationResult.success) {
                return res.status(400).json({
                    success: false,
                    message: validationResult.error.errors[0].message,
            ...
            let existingUser = yield user_1.User.findById(userId);
            if (!existingUser) {
                return res.status(401).json({
                    success: false,
            ...
            const isPasswordEqual = yield (0, auth_controller_1.comparePasswords)({
                password: password,
                hashedPassword: existingUser.password.hashed,
                salt: existingUser.password.salt
            });
            ...
            let newPasswordHash = yield (0, auth_controller_1.hashPassword)(newPassword, newPasswordSalt);
...
router.post("/create-forget-password-session", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Validate email
            const validationResult = yield schemaComponents_1.emailValidatior.safeParseAsync(req.body.email || "");
            if (!validationResult.success) {
                return res.status(400).json({
                    success: false,
                    message: validationResult.error.errors[0].message,
...
router.post("/request-forget-password-otp", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let validationResult = yield auth_schema_1.tempSessionValidation.safeParseAsync(req.body.sessionKey);
            if (!validationResult.success) {
            ...
            // Store OTP in session
            session.value = JSON.stringify(Object.assign(Object.assign({}, sessionData), { otp, otpExpiry: Date.now() + 70 * 1000 // OTP valid for 1 minute 10 seconds
             }));
            ...
            // Send OTP via email
            const emailSent = yield auth_emails_1.authEmails.forgotPasswordOtpEmail(otp, sessionData.email);
            if (!emailSent) {
                return res.status(500).json({
                    success: false,
                    message: "Failed to send OTP",
...
router.post("/verify-forget-password-otp", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let validationResult = yield auth_schema_1.VerifyForgotPasswordOtpSchema.safeParseAsync(req.body);
            if (!validationResult.success) {
                return res.status(400).json({
                    success: false,
                    message: validationResult.error.errors[0].message,
            ...
            const user = yield user_1.User.findById(sessionData.userId);
            if (!user) {
                return res.status(404).json({
                    success: false,
            ...
            const passwordHash = yield (0, auth_controller_1.hashPassword)(newPassword, passwordSalt);
...
// Verify forget password OTP and reset password
router.post("/log-out", auth_middleware_1.validateUser, function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b;
        try {
            ...
            yield ((_b = req.authSession) === null || _b === void 0 ? void 0 : _b.deleteOne());
            res.status(200).json({
                success: true,
...
router.post("/create-registration-session/video-profile", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const videoProfileRegistrationSchema = zod_1.z.object({
                ...
                .transform((data) => (Object.assign(Object.assign({}, data), { age: Math.floor((new Date().getTime() - data.dateOfBirth.getTime()) / (365.25 * 24 * 60 * 60 * 1000)) })));
            const validationResult = yield videoProfileRegistrationSchema.safeParseAsync(req.body);
            if (!validationResult.success) {
                return res.status(400).json({
                    success: false,
                    message: validationResult.error.errors[0].message,
            ...
            let existingUser = yield VideoProfile_1.default.findOne({ email: userData.email });
            if (existingUser) {
                return res.status(400).json({
                    success: false,
                    message: "Email is already registered",
...
router.post("/request-registration-otp/video-profile", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let validationResult = yield auth_schema_1.tempSessionValidation.safeParseAsync(req.body.sessionKey);
            if (!validationResult.success) {
            ...
            userData.hasOtpRequest -= 1;
            // Store OTP in session
            session.value = JSON.stringify(Object.assign(Object.assign({}, userData), { otp, otpExpiry: Date.now() + 300 * 1000 // OTP valid for 5 minutes
             }));
            // Handle session based on remaining OTP requests
            ...
            // Send OTP via email
            let emailSent = yield auth_emails_1.authEmails.registrationOtpEmail(otp, userData.email);
            if (!emailSent) {
                res.status(400).json({
                    success: false,
...
router.post("/verify-registration-otp/video-profile", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const validationResult = yield auth_schema_1.VerifyOtpSchema.safeParseAsync(req.body);
            if (!validationResult.success) {
                return res.status(400).json({
                    success: false,
                    message: validationResult.error.errors[0].message,
            ...
            const passwordSalt = (0, auth_controller_1.generateSalt)();
            const passwordHash = yield (0, auth_controller_1.hashPassword)(sessionData.password, passwordSalt);
            const authToken = crypto_1.default.randomBytes(32).toString('hex').normalize();
            // Create new video profile user
            const newUser = yield VideoProfile_1.default.create({
                name: sessionData.name,
                email: sessionData.email,
                passwordDetails: {
                    hashed: passwordHash,
...
router.post('/is-registration-successfull/video-profile', function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let schema = zod_1.z.object({
                userId: zod_1.z.optional(schemaComponents_1._idValidator),
            ...
            let user;
            if (userId) {
                user = yield VideoProfile_1.default.findById(userId);
                if (!user) {
                    return res.status(400).json({
                        success: false,
            ...
            if (authToken) {
                let user = yield VideoProfile_1.default.findOne({ 'auth.authSession': authToken,
...
router.post('/login/video-profile', function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let { credential, password } = req.body;
            if (!credential || !password) {
            ...
            if (emailRegex.test(credential)) {
                ...
                existingUser = yield VideoProfile_1.default.findOne({ email: credential });
            }
            else {
                if (phoneRegex.test(credential) === false) {
                    return res.status(400).json({
                ...
                loginType = auth_schema_1.LoginEnum.withPhone;
                existingUser = yield VideoProfile_1.default.findOne({
                    phone: credential
                });
            }
            if (!existingUser) {
...
router.post("/create-forget-password-session/video-profile", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        try {
            // Validate email
            const validationResult = yield schemaComponents_1.emailValidatior.safeParseAsync((_a = req.body) === null || _a === void 0 ? void 0 : _a.email);
            if (!validationResult.success) {
                return res.status(400).json({
                    success: false,
                    message: validationResult.error.errors[0].message,
            ...
            const email = validationResult.data;
            // Check if user exists
            const existingUser = yield VideoProfile_1.default.findOne({ email });
            if (!existingUser) {
                return res.status(404).json({
                    success: false,
...
router.post("/request-forget-password-otp/video-profile", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let validationResult = yield auth_schema_1.tempSessionValidation.safeParseAsync(req.body.sessionKey);
            if (!validationResult.success) {
            ...
            sessionData.hasOtpRequest -= 1;
            // Store OTP in session
            session.value = JSON.stringify(Object.assign(Object.assign({}, sessionData), { otp, otpExpiry: Date.now() + 300 * 1000 // OTP valid for 5 minutes
             }));
            // Handle session based on remaining OTP requests
            if (sessionData.hasOtpRequest < 1) {
                yield session.deleteOne();
            ...
            // Send OTP via email
            const user = yield VideoProfile_1.default.findById(sessionData.userId);
            if (!user) {
                res.status(400).json({
                    success: false,
            ...
            let isEmailSent = yield auth_emails_1.authEmails.forgotPasswordOtpEmail(otp, user.email);
            if (!isEmailSent) {
                res.status(400).json({
                    success: false,
...
router.post("/verify-forget-password-otp/video-profile", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const validationResult = yield auth_schema_1.VerifyForgotPasswordOtpSchema.safeParseAsync(req.body);
            if (!validationResult.success) {
                return res.status(400).json({
                    success: false,
                    message: validationResult.error.errors[0].message,
...
router.post("/reset-password/video-profile", function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const validationResult = yield auth_schema_1.ResetPasswordSchema.safeParseAsync(req.body);
            if (!validationResult.success) {
                return res.status(400).json({
                    success: false,
                    message: validationResult.error.errors[0].message,
            ...
            // Find user
            const existingUser = yield VideoProfile_1.default.findById(userId);
            if (!existingUser) {
                return res.status(401).json({
                    success: false,
...
router.post("/log-out/video-profile", auth_middleware_1.validateVideoProfile, function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        try {
            if (!req.videoProfile && !((_a = req.videoProfile) === null || _a === void 0 ? void 0 : _a._id)) {
                res.status(401).json({
                    success: false,
                    message: 'Invalid request parameters',
                    data: null
            ...
            let updatedUser = yield VideoProfile_1.default.updateOne({
